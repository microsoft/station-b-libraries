...
# -------------------------------------------------------------------------------------------
"""Patch to multi-point Expected Improvement."""
...
def _multinormal_cdf(x: np.ndarray, mean, cov) -> float:
    x = np.asarray(x)
    raw = scipy.stats.multivariate_normal.cdf(x, mean, cov)
    if not np.isnan(raw):
        return raw
    else:
        # If NaN, we check if all coordinates passed the mean (then CDF = 1). If not, then CDF = 0.
        return float(np.all(x > mean))
...
class MultipointExpectedImprovement(ExpectedImprovement):
    ...
    def _get_acquisition(self, mu: np.ndarray, Sigma: np.ndarray, y_minimum: float) -> Tuple:
        ...
        for k in range(q):
            ...
            pk[k] = _multinormal_cdf(b_k - mu_k, np.zeros((q,)), Sigma_k)
    ...
    def _get_non_symmetric_and_symmetric_term_k(
        ...
        if self.fast_compute:
            non_symmetric_term[k] = (
                1.0
                / self.eps
                * (_multinormal_cdf(b_k - mu_k + self.eps * Sigma_k[:, k].flatten(), np.zeros((q,)), Sigma_k) - pk[k])
            )
            ...
            for i in range(q):
                ...
                if i >= k:
                    ...
                    Phi_ik = _multinormal_cdf(cik, np.zeros((q - 1,)), sigmaik)
    ...
    def _gradient_of_the_acquisition_first_term(
        ...
        for i, j in itertools.product(range(q), range(d)):
            Sigk_dx[i, j, :, :] = Lk @ dSigma_dx[:, :, i, j] @ tLk
            mk_dx[i, j, :] = Lk @ dmu_dx[:, i, j]
            Dpk[i, j] = 0.5 * np.sum(hesspk * Sigk_dx[i, j, :, :]) - gradpk[None, :] @ mk_dx[i, j, :, None]
    ...
    def _gradient_of_the_acquisition_second_term(
        ...
        if self.fast_compute:
            ...
            for i, j in itertools.product(range(q), range(d)):
                f1 = (
                    -(mk_dx[None, i, j, :] @ gradpk1[:, None])
                    + self.eps * (Sigk_dx[None, i, j, :, k] @ gradpk1)
                    + 0.5 * np.sum(Sigk_dx[i, j, :, :] * hesspk1)
                )
                f = -(mk_dx[None, i, j, :] @ gradpk[:, None]) + 0.5 * np.sum(Sigk_dx[i, j, :, :] * hesspk)
                B[i, j] = 1.0 / self.eps * (f1 - f)
        else:
            ...
            for i in range(q):
                ...
                for i, j in itertools.product(range(q), range(d)):
                    ...
                    B1[i, j] = B1[i, j] + Sigk_dx_ik[i, j] * phi_ik * Phi_ik

                    # B2: Third row of Equation (6) in the paper
                    B2[i, j] = (
                        B2[i, j] + Sigk_ik * (mk_dx_i[i, j] * dphi_ik_dm + dphi_ik_dSig * Sigk_dx_ii[i, j]) * Phi_ik
                    )

                    # B3: Fourth row of Equation (6) in the paper
                    dck_pi = (
                        -mk_dx[i, j, ineq]
                        + (mk_dx_i[i, j] * Sigk_ii + (bk_i - mk_i) * Sigk_dx_ii[i, j]) / (Sigk_ii ** 2) * Sigk_mi[:, 0]
                        - (bk[i, 0] - mk_i) / Sigk_ii * Sigk_dx[i, j, ineq, i]
                    )
                    SigtCross = Sigk_dx[i, j, ineq, i, None] @ Sigk_mi.T
                    dSigk_pi = (
                        Sigk_dx[i, j, :, :][:, ineq][ineq, :]
                        + Sigk_dx_ii[i, j] / (Sigk_ii ** 2) * (Sigk_mi @ Sigk_mi.T)
                        - (1.0 / Sigk_ii) * (SigtCross + SigtCross.T)
                    )
                    B3[i, j] = B3[i, j] + Sigk_ik * phi_ik * ((GPhi_ik.T @ dck_pi) + 0.5 * np.sum(HPhi_ik * dSigk_pi))
            B = B1 + B2 + B3
        return B
